...
import { memberships, merchants, sessions, setTenantContext, users } from '@shopai/db';
...
export type AuthenticatedMerchant = { id: string; name: string; slug: string; role: Role };
declare module 'fastify' {
  interface FastifyRequest { auth: AuthContext | null }
}

const legacyCookie = 'shopai_session';
const oauthCookie = (env: ApiEnv) => env.DEPLOY_ENV === 'local' || env.DEPLOY_ENV === 'test'
  ? 'shopai_oidc_session' : '__Host-shopai_session';
...
const safeEqual = (a: string, b: string) => timingSafeEqual(Buffer.from(hash(a), 'hex'), Buffer.from(hash(b), 'hex'));
...
export interface AuthApi {
  ...
  issueOidcSession(userId: string, kind: Auth0ClientKind, level: 'password' | 'mfa', authenticatedAt: Date | null, reply: FastifyReply): Promise<void>;
  csrfToken(request: FastifyRequest): string | null;
  verifyPilotCredential(email: string, token: string): boolean;
  membership(request: FastifyRequest, merchantId: string): Promise<Role | undefined>;
...
export function registerAuth(app: FastifyInstance, db: Database | undefined, env: ApiEnv, oidcEnabled = false, pilotEnabled = true) {
  ...
  const authApi: AuthApi = {
    ...
    verifyPilotCredential(email: string, token: string) {
      ...
      return safeEqual(configured ?? 'invalid-credential-placeholder', token) && Boolean(configured);
    },
    async login(email: string, token: string, reply: FastifyReply) {
      if (!pilotEnabled) return reply.code(410).send({ code: 'PILOT_LOGIN_DISABLED' });
      ...
      const existing = await database.execute(sql`SELECT id, account_status FROM users WHERE email = ${normalized}`);
      if (existing.rows.length && existing.rows[0]?.account_status !== 'pilot')
        return reply.code(403).send({ code: 'PILOT_ACCOUNT_MIGRATED' });
      const [user] = await database.insert(users).values({ email: normalized })
        .onConflictDoNothing({ target: users.email }).returning();
      const id = user?.id ?? existing.rows[0]?.id;
      if (typeof id !== 'string') return reply.code(409).send({ code: 'PILOT_LOGIN_CONFLICT' });
      const raw = randomBytes(32).toString('base64url');
      await database.insert(sessions).values({
        userId: id, tokenHash: hash(raw),
        expiresAt: new Date(Date.now() + env.SESSION_TTL_HOURS * 3600000),
      });
      reply.header('Set-Cookie', `${legacyCookie}=${raw}; ${cookieAttributes(env)}; Max-Age=${env.SESSION_TTL_HOURS * 3600}`);
    ...
    async authenticate(request: FastifyRequest) {
      ...
      if (!row || typeof row.user_id !== 'string' || typeof row.email !== 'string') return null;
      return {
        userId: row.user_id,
        email: row.email,
        authLevel: row.auth_level as AuthContext['authLevel'],
        authenticatedAt: row.authenticated_at instanceof Date ? row.authenticated_at : null,
    ...
    csrfToken(request) {
      ...
      return request.auth?.authLevel !== 'pilot' && validSessionToken(raw)
        ? createHmac('sha256', env.REDIRECT_SIGNING_SECRET).update(raw).digest('base64url') : null;
    },
    async logout(request, reply) {
      const raw = cookie(request, oauthCookie(env)) ?? cookie(request, legacyCookie);
      if (validSessionToken(raw) && db)
        await db.execute(sql`UPDATE sessions SET revoked_at = now() WHERE token_hash = ${hash(raw)} AND revoked_at IS NULL`);
      clearCookies(reply);
      return { ok: true };
    },
    async logoutAll(request, reply) {
      if (!request.auth) return reply.code(401).send({ code: 'UNAUTHENTICATED' });
      await requireDb().execute(sql`UPDATE sessions SET revoked_at = now() WHERE user_id = ${request.auth.userId}::uuid AND revoked_at IS NULL`);
      clearCookies(reply);
      return { ok: true };
    },
    async membership(request, merchantId) {
      ...
      return db.transaction(async (tx) => {
        ...
        const [membership] = await tx.select({ role: memberships.role }).from(memberships)
          .where(and(eq(memberships.userId, request.auth!.userId), eq(memberships.merchantId, merchantId)));
        return membership?.role as Role | undefined;
      });
    },
    async listMerchants(request) {
      if (!request.auth || !db) return [];
      const rows = await db.select({ id: merchants.id, name: merchants.name, slug: merchants.slug, role: memberships.role })
        .from(memberships).innerJoin(merchants, eq(merchants.id, memberships.merchantId))
        .where(eq(memberships.userId, request.auth.userId)).orderBy(asc(merchants.name), asc(merchants.id));
  ...
  app.addHook('preHandler', async (request, reply) => {
    ...
    if (!request.auth || request.auth.authLevel === 'pilot' || !['POST','PUT','PATCH','DELETE'].includes(request.method)) return;
    ...
    if (path === '/mcp' || path === '/v1/search' || path === '/discovery-session' ||
      path === '/v1/interaction-events' || path?.match(/^\/v1\/stores\/[^/]+\/search$/u)) return;
    const origin = request.headers.origin;
    const expected = authApi.csrfToken(request);
    if (!origin || !authApi.allowedOrigins.has(origin) || !expected ||
...
export const requireRole = (...allowed: Role[]) =>
  async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    ...
    const role = merchantId ? await request.server.authApi.membership(request, merchantId) : undefined;
    if (!request.auth) return void reply.code(401).send({ code: 'UNAUTHENTICATED' });
    if (!role || !allowed.includes(role)) return void reply.code(403).send({ code: 'FORBIDDEN' });
    if (role === 'owner' && request.auth.authLevel !== 'pilot' && request.auth.authLevel !== 'mfa')
      return void reply.code(403).send({ code: 'MFA_REQUIRED' });
  };

export const requireRecentMfa = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
  const auth = request.auth;
  if (!auth || auth.authLevel !== 'mfa' || !auth.authenticatedAt || Date.now() - auth.authenticatedAt.getTime() > 5 * 60000)
    return void reply.code(403).send({ code: 'MFA_STEP_UP_REQUIRED' });
};

export const requireSameOrigin = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
  const origin = request.headers.origin;
