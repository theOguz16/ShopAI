...
import type { Auth0ClientKind, Auth0Config, VerifiedIdentity } from './auth0-oidc.js';
...
export async function verifyAuth0Grant(
  ...
  const configuration = await oidc.discovery(
    new URL(config.issuer), client.clientId, client.clientSecret,
    oidc.ClientSecretPost(client.clientSecret),
  );
  if (configuration.serverMetadata().issuer !== config.issuer) throw new Error('OIDC_INVALID_ISSUER');
  for (const endpoint of [
    ...
    if (typeof endpoint !== 'string' || new URL(endpoint).origin !== new URL(config.issuer).origin || new URL(endpoint).protocol !== 'https:')
  ...
  if (!rawClaims || typeof rawClaims !== 'object') throw new Error('OIDC_MISSING_CLAIMS');
  const claims = rawClaims as Record<string, unknown>;
  if (claims.iss !== config.issuer || claims.aud !== client.clientId &&
      (!Array.isArray(claims.aud) || !claims.aud.includes(client.clientId)))
    throw new Error('OIDC_WRONG_AUDIENCE');
  if (claims.email_verified !== true || typeof claims.sub !== 'string' || !claims.sub || claims.sub.length > 512)
  ...
  const recent = typeof time === 'number' && time <= now + 60 && time >= now - 5 * 60;
  ...
  const mfa = recent && ((Array.isArray(amr) && amr.includes('mfa')) ||
  ...
  return {
    ...
    emailVerified: true,
    mfa,
    authenticatedAt: typeof time === 'number' && time > 0 && time <= now + 60
