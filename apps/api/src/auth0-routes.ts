...
const resetSchema = z
  ...
  .strict();
const bindingName = (env: ApiEnv) =>
  env.DEPLOY_ENV === 'staging' || env.DEPLOY_ENV === 'production'
    ? '__Host-shopai_oidc_state'
    : 'shopai_oidc_state';
const attributes = (env: ApiEnv) =>
...
const authError = (reply: FastifyReply) =>
  reply.code(401).send({ code: 'OIDC_AUTHENTICATION_FAILED' });

export function registerAuth0Routes(
  app: FastifyInstance,
  ...
  async function begin(
    ...
    try {
      authorizationEndpoint = (await discover(config!)).authorizationEndpoint;
    } catch {
    ...
    reply.header(
      'Set-Cookie',
      `${bindingName(env)}=${binding}; ${attributes(env)}; Max-Age=300`,
    );
    const authorization = authorizationUrl(
      ...
      input.client,
      {
        ...
        nonce,
        verifier,
        signup: input.signup === 'true',
        stepup: flowKind === 'stepup',
      },
    );
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (
        !app.authApi.verifyPilotCredential(normalized, parsed.data.pilotToken)
      )
        return reply.code(401).send({ code: 'INVALID_CREDENTIALS' });
      const rows = await database.execute(
        sql`SELECT id FROM users WHERE email = ${normalized} AND account_status = 'pilot'`,
      );
      const id = rows.rows[0]?.id;
      if (typeof id !== 'string')
        return reply.code(409).send({ code: 'PILOT_ACCOUNT_NOT_FOUND' });
      return begin(
        request,
        reply,
        { client: parsed.data.client, returnTo: parsed.data.returnTo },
        id,
      );
    },
  );
  async function resolveAccount(
    ...
    return database.transaction(async (tx) => {
      ...
      if (existing) {
        ...
        if (record.claimUserId && record.claimUserId !== userId)
          throw new Error('IDENTITY_CONFLICT');
      } else if (record.claimUserId && record.flowKind === 'login') {
        userId = record.claimUserId;
        const account = await tx.execute(
          sql`SELECT email,account_status FROM users WHERE id = ${userId}::uuid FOR UPDATE`,
        );
        if (
          account.rows[0]?.account_status !== 'pilot' ||
          account.rows[0]?.email !== identity.email
        )
        ...
        await tx.execute(
          sql`UPDATE users SET account_status='active', email_verified_at=now() WHERE id=${userId}::uuid`,
        );
        await tx.execute(
          sql`UPDATE sessions SET revoked_at=now() WHERE user_id=${userId}::uuid AND auth_level='pilot' AND revoked_at IS NULL`,
        );
        await tx.execute(
          sql`INSERT INTO auth_audit_events (user_id,event_type,outcome,request_id) VALUES (${userId}::uuid,'identity_link','success',${requestId})`,
        );
      } else {
        if (record.flowKind === 'stepup')
        ...
        const collision = await tx.execute(
          sql`SELECT 1 FROM users WHERE email=${identity.email} LIMIT 1`,
        );
      ...
      await tx.execute(
        sql`INSERT INTO auth_audit_events (user_id,event_type,outcome,request_id) VALUES (${userId}::uuid,${record.stepupUserId ? 'mfa' : 'login'},'success',${requestId})`,
      );
      return userId;
    });
  }
  app.get(
    '/v1/auth/oidc/callback/:kind',
    {
    ...
    `);
      ...
      try {
        const verifier = decrypt(
          String(record.encrypted_verifier),
          config.encryptionKey,
        );
        const nonce = decrypt(
          String(record.encrypted_nonce),
          config.encryptionKey,
        );
        if (sha256(nonce) !== String(record.nonce_hash))
          throw new Error('OIDC_NONCE_MISMATCH');
        const identity = await verifyAuth0Grant(
          config,
          kind as Auth0ClientKind,
          parsed.data.code,
          parsed.data.state,
          nonce,
          verifier,
        ...
        const userId = await resolveAccount(
          ...
          {
            claimUserId:
              typeof record.claim_user_id === 'string'
                ? record.claim_user_id
                : null,
            stepupUserId:
              typeof record.stepup_user_id === 'string'
                ? record.stepup_user_id
                : null,
        ...
        if (record.stepup_user_id && request.auth?.sessionId)
          await database.execute(
            sql`UPDATE sessions SET revoked_at=now() WHERE id=${request.auth.sessionId}::uuid AND user_id=${userId}::uuid`,
          );
        await app.authApi.issueOidcSession(
          userId,
          kind as Auth0ClientKind,
          identity.mfa ? 'mfa' : 'password',
          identity.authenticatedAt,
          reply,
        );
        return reply.redirect(
          `${config.webOrigin}${String(record.return_to)}`,
          303,
        );
      } catch {
        return authError(reply);
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        /* never reflect provider details */
      }
      return reply.code(202).send({ ok: true });
    },
  );
  app.post('/v1/auth/logout-all', async (request, reply) =>
    app.authApi.logoutAll(request, reply),
  );
  app.post(
