...
function shopai_connector_create_api_key() {
	...
	update_option( SHOPAI_CONNECTOR_KEY_ID_OPTION, $key_id );
...
function shopai_connector_delete_api_key() {
	global $wpdb;
	$key_id = (int) get_option( SHOPAI_CONNECTOR_KEY_ID_OPTION, 0 );
	if ( $key_id ) {
		$wpdb->delete(
			$wpdb->prefix . 'woocommerce_api_keys',
			array( 'key_id' => $key_id ),
...
function shopai_connector_handle_connect() {
	...
	if ( is_wp_error( $key ) ) {
		wp_safe_redirect( add_query_arg( 'shopai_result', 'key_failed', $redirect ) );
		exit;
	}
	...
	if ( 200 !== $status && 201 !== $status ) {
		...
		shopai_connector_delete_api_key();
	...
	if ( ! is_array( $body ) || empty( $body['connection']['id'] ) ) {
		shopai_connector_delete_api_key();
		wp_safe_redirect( add_query_arg( 'shopai_result', 'unexpected_response', $redirect ) );
		exit;
	}
	...
	wp_safe_redirect( add_query_arg( 'shopai_result', 'connected', $redirect ) );
	exit;
}
...
function shopai_connector_handle_disconnect_local() {
	...
	$redirect = admin_url( 'admin.php?page=shopai-connector' );
	wp_safe_redirect( add_query_arg( 'shopai_result', 'local_disconnected', $redirect ) );
	exit;
}
