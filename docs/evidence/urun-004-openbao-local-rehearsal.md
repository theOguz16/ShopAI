...
- 2.7.0 `disable_mlock` ayarını reddetti; bu beklenen davranıştır: OpenBao 2.0.0'dan itibaren mlock işlevi kaldırılmıştır ve `disable_mlock` obsolete bir seçenektir. OpenBao mlock kullanmadığı için container'a `IPC_LOCK` capability'si gerekmez ve verilmez; config'de `disable_mlock` bulunmaz. Named Raft volume'ün UID 100 OpenBao kullanıcısına ait olması gerektiği canlı startup'ta görüldü; Compose'a tek seferlik volume ownership init servisi eklendi.
