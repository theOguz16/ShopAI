...
| Arayüz ve cutover | Giriş, kayıt, doğrulama, TOTP/backup, claim | PR #59 exact SHA staging'de, pilot giriş açık. İlk gerçek doğrulama bağlantısı eksik `callbackURL` nedeniyle API `/` yoluna yönlenip 404 gösterdi; regresyon ve UI düzeltmesi hazırlandı, staging'de yeniden doğrulama, MFA, hesap bağlama ve geri dönüş kabulü açık. |
