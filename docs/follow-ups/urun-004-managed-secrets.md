...
Operatör özel CA ve `openbao.shopai.internal` SAN içeren sunucu sertifikasını hostta erişim kontrollü dosyalardan read-only mount eder; OpenBao container UID 100 TLS private key'i okuyabilmelidir. Staging ve production için KV v2 mount, health sentinel, ayrı AppRole ve audit device kurar. API/worker RoleID ayrı env'de; SecretID ayrı host dosyasında tutulur ve container'a read-only mount edilir. SecretID dosyası rotation'da atomik değiştirilir; adapter her işlemde dosyayı yeniden okuyup kısa ömürlü AppRole token alır. Token loglanmaz. SecretID 24 saat TTL ile döndürülür; her işlemde yeniden login yapıldığı için usage limit 0'dır. Gerçek private source CIDR'leri doğrulanınca rol login erişimi bunlarla sınırlanır. SecretID operatör gözetiminde response wrapping ile dağıtılır. Root token yalnız bootstrap sırasında operatörün güvenli oturumunda kullanılır, sonra off-host admin identity kurularak revoke edilir; repo, uygulama env'i veya kalıcı VDS dosyasına yazılmaz.
...
`scripts/migrate-connector-secrets.mts`: `plan → OpenBao write → OpenBao read-back verify → PostgreSQL transaction/reference switch`. `--apply` local/staging ile sınırlı; aynı active backend tekrar taşınmaz. DB yarışında geçici OpenBao kaydı temizlenir. `--rollback` explicit operatör recovery eylemidir; production'da da kullanılabilir. Eski scoped file kaydı, merchant/connection/provider/sürüm ve migration audit'i doğrulanır; OpenBao health gerekmez. DB switch transaction içinde yapılır, OpenBao kaydına dokunulmaz ve `migration-rollback-openbao-cleanup-pending` audit actor'ı ile orphan cleanup bekler. İkinci çalıştırma değişiklik yapmaz. Plaintext/unscoped pilot dosyaya otomatik rollback yoktur. Production API/worker OpenBao-only kalır; file referansına runtime fallback yapmaz. Worker'ın eski file referansını çözmesi için ayrıca onaylı eski file-backend release/config rollback'i gerekir. `--cleanup` 30 gün retention sonrası aktif OpenBao secret'ı doğrulayan **dry-run**'dır ve dosya silmez. Ayrı `--cleanup --confirm-retired-file-deletion` emekli dosyayı siler. Filesystem unlink, fiziksel blok ve backup kopyalarının güvenli silinmesini kanıtlamaz. Production migration çalıştırılmaz.
