...
- [x] Local/staging file connector credential'ları AES-256-GCM ile encrypted-at-rest tutuluyor; production yalnız AWS Secrets Manager backend'i kabul ediyor.
- [x] Production API/worker AWS backend config veya health secret okuma izni olmadan başlamıyor.
...
Yerel AES-256-GCM backend production kabulü değildir. AWS Secrets Manager adapter kodu vardır; gerçek ayrı staging provider kabulü, IAM/key rotation, pilot plaintext backup temizliği ve recovery tatbikatı henüz kanıtlanmadı. [Issue #9](https://github.com/theOguz16/ShopAI/issues/9) açık kalır.
...
PRODUCTION_CONNECTOR_SECRET_AWS_REGION=...
PRODUCTION_CONNECTOR_SECRET_AWS_HEALTH_SECRET_ID=shopai/production/health
PRODUCTION_AWS_API_ACCESS_KEY_ID=...
PRODUCTION_AWS_API_SECRET_ACCESS_KEY=...
PRODUCTION_AWS_WORKER_ACCESS_KEY_ID=...
PRODUCTION_AWS_WORKER_SECRET_ACCESS_KEY=...
