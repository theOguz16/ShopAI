...
- TASK-023A sentetik pilot prova harness'i; ayrı PostgreSQL veritabanında 5 sentetik merchant, tek merchantta 10k+ ürün, 100 deterministik web/ChatGPT-shaped yolculuk, TASK-022 metrik semantiği, gerçek redirect/conversion/analytics yolları ve deterministik chaos kontrolleri için sürümlü JSON + metin raporu üretir. Bu otomatik kanıt gerçek merchant/kullanıcı kabulü değildir; TASK-023B açık kalır.
...
- Catalog Health dashboard `GET /v1/merchants/:merchantId/catalog-health` üzerinden tenant-scoped PostgreSQL aggregate'leri kullanır; toplam/yayındaki/aktif/stokta/eksik görsel/eksik fiyat metrikleri browser mock'u veya tüm ürün listesini istemciye indirmeden gerçek `products`, `variants`, `offers` ve `inventory` kayıtlarından hesaplanır. Aktif ürün, yayımlanmış ve en az bir aktif offer'ı olan üründür; fiyat kolonu DB'de zorunlu olduğu için eksik fiyat, aktif offer'ı bulunmayan ürün anlamına gelir.
- Catalog Health connection durumu `healthy/stale/pending/attention` olarak commerce iş kuralından üretilir. Başarılı sync yaşı 30 dakikayı geçtiğinde worker'ın mevcut `catalog_stale` operasyon politikasıyla aynı eşikte stale uyarısı gösterilir; dashboard son başarılı sync yaşını ve `Catalog may be stale` uyarısını gerçek connection timestamp'lerinden sunar.
- Public Product Discovery API için `SearchProductsRequest` / `SearchProductsResponse` tek consumer-facing sözleşme olarak `@shopai/contracts` içinde tanımlıdır. Request; discovery session, merchant scope, query, kategori, fiyat, attribute, stok, cursor ve limit alanlarını; response ise `products`, `facets`, opsiyonel `nextCursor` ve `searchId` alanlarını taşır. İç repository/search filtre şekli public transport sözleşmesine sızdırılmaz.
- Web/REST ve ChatGPT/MCP canonical request'leri aynı `executePublicSearch` servis sınırından ve aynı `@shopai/commerce` adaptöründen geçer. Parity integration testi aynı request'i REST ve MCP üzerinden çalıştırıp ürün/facet result set'inin eşitliğini doğrular; transport/surface attribution nedeniyle imzalı `checkoutUrl` farklılaşabildiği için commerce result identity karşılaştırmasından yalnız bu transport-özel alan çıkarılır.
- Category + Facet MVP, kategori metadata'sını `categories`, kategoriye bağlı facet tanımlarını `category_facets`, ürünün kategoriye özgü değerlerini `product_attributes` tablolarında tutar. Pilot seed yalnız `tshirt` (`size`, `color`, `fit`, `sleeve`) ve `fishing-rod` (`length`, `action`, `casting_weight`) kategorilerini tanımlar; generic ontology/schema engine yoktur. `GET /categories/:slug/facets` facet seçeneklerini DB'den okuyup commerce katmanında doğrulanmış map olarak döndürür.
- Product Detail consumer sözleşmesi `ProductDetailResponse` ile ürün, kaynakta varsa görseller, açıklama, varyantlar, aktif offer'lar, availability, kategoriye özgü attributes, public merchant bilgisi, checkout uygunluğu ve aynı merchant/kategoriden benzer ürünleri taşır. `GET /v1/products/:productId/detail` ile MCP `get_product_detail` aynı commerce use-case'ini kullanır. Kaynak katalogda brand alanı henüz olmadığı için brand uydurulmaz ve `null` döner. Varyant yalnız en az bir güncel `in_stock` offer'ı varsa `selectable=true` olur; `out_of_stock`, `unknown` ve `stale` varyantlarda checkout linki üretilmez. Next.js detail ekranı ve ChatGPT widget renk/beden seçimini bu server-derived `selectable` alanıyla disable eder.
