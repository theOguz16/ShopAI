...
function change(path, oldText, newText) {
  ...
  if (
    ...
    source.indexOf(oldText) !== source.lastIndexOf(oldText)
  )
...
const line = parts.findIndex(
  (part) =>
    part.includes('u0000') &&
    part.includes('u001f') &&
    part.includes('test(value)'),
);
...
console.log(
  'Prepared proof-bound redirect, normalized return paths and source validation',
);
